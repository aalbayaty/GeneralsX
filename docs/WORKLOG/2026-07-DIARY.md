...
### MinGW Docker Image: Missing IDL Compiler
- Fixed the MinGW cross-build failing at configure time: `EABrowserDispatch` requires Wine's IDL compiler (`widl`), but `resources/dockerbuild/Dockerfile.mingw` only installed `wine64`.
- Added `wine64-tools` (provides `widl`) and `libwine-dev` (provides the Wine IDL headers such as `oaidl.idl` probed by `cmake/widl.cmake`) to the builder image.

### Restored the MinGW Windows Build (Mac↔Windows Crossplay Path)
The `mingw-w64-i686` target had fully bit-rotted (never CI-built since the SDL3/OpenAL migration). Fixed end-to-end until `generalszh.exe` (PE32) builds in Docker. All changes are guarded to leave macOS/Linux builds byte-identical:
- **Toolchain determinism**: `-msse2 -mfpmath=sse` in the MinGW toolchain file — i686 GCC otherwise defaults to x87 80-bit math, which would desync CRC lockstep against x86_64/ARM64 clients.
- **Builder image**: bumped to Ubuntu 24.04 (MinGW GCC 13) for C++20 float `std::from_chars` (INI parsing, determinism-critical) and atomic wait/notify.
- **CMake configuration**: `SAGE_USE_DX8` now defaults ON for WIN32 (previously undefined → DXVK Linux headers shadowed the real SDK); SagePatch gated on `SAGE_USE_SDL3`; d3dx8 duplicate-target guard in CompatLib; Win32 input sources compiled from Core like upstream; stale `Win32CDManager`/game-level refs removed; Miles/Bink/FramGrab re-enabled for WIN32 (stub SDKs at link, retail DLLs at runtime); CompatLib include dirs (fake `windows.h`) excluded from native Windows builds.
- **Source fixes**: WWAudio `mmsystem.h` stub now `#include_next`s the real SDK header on MinGW; restored dropped `<time.h>`/`<sys/stat.h>` and `socklen_t` typedefs in Winsock TUs (FTP.cpp, udp.h, PeerThread); `mutex.h` atomic wait/notify feature-gated for pre-C++20 libstdc++; `_isnan`/`_finite` resolved via `<float.h>` on MinGW (intrin_compat); `KF_FLAG_DEFAULT`/`FOLDERID_Documents` fallbacks extended to MinGW; deep-CRC dump header and `setFPMode()` use portable branches on MinGW; `TheSystemIsUnicode` declared at its Core use site and defined in Generals (backport); `__int64` typedef guards for command-line macro builds (profile/debug headers).
- **Build script**: `set -eo pipefail` (was reporting success on failure) and corrected artifact name (`generalszh.exe`).

### Removed d3dx8d.dll Runtime Dependency (MinGW)
- The MinGW exe imported `d3dx8d.dll` (DX8 SDK debug DLL, not redistributable) via MinGW's `libd3dx8d.a` import library, so it could not run on a normal Windows machine.
- Replaced with GeneralsX's own from-source D3DX8 implementation (`CompatLib/d3dx8`, the same GLM/GLI code the macOS/Linux builds use): now also compiled on MinGW against the real SDK headers, with glm 0.9.9.8 / pinned gli master fetched via FetchContent (no vcpkg in the cross build).
- Removed the `d3dx8 -> d3dx8d` alias in `cmake/mingw.cmake`; min-dx8-sdk's bare `d3dx8d` link reference is rerouted to the static implementation via a forwarding INTERFACE target in `cmake/dx8.cmake`.
- Fixed a latent bug: `d3dx8_compat.cpp` declared `WINAPID3DXGetErrorStringA` (missing space), silently defining a dead function on all platforms; removed (a correct definition already existed below).
- Verified: `objdump -p generalszh.exe` shows no `d3dx8d.dll` import; only system DLLs plus the retail `mss32.dll`/`binkw32.dll` remain.
